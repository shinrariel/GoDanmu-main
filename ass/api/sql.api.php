...
class sqlobj {
    ...
    private $db_pwd; //字符串字符串型，型，数据库用户名密码
    private $db_database; //数据库名
    private $conn; //指针型，原生数据库连接标识;
    private $result; //其它型，执行query命令的结果资源标识
    private $sql; //字符串型，sql执行语句
    private $row; //整型，返回的条目数
    private $coding; //字符串型，数据库编码，GBK,UTF8,gb2312
    private $bulletin = true; //布尔型，是否开启错误记录
    private $show_error = false; //布尔型，测试阶段，显示所有错误,具有安全隐患,默认关闭
    private $is_error = false; //布尔型，发现错误是否立即终止,默认true,建议不启用，因为当有问题时用户什么也看不到是很苦恼的
